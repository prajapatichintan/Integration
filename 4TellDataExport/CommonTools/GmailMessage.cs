...
namespace _4_Tell.Utilities.Gmail
{
	...
	public class GmailMessage : MailMessage 
	{
		...
		public void Send(string fromDisplay = null, string fromId = null) 
		{
			try 
			{
				...
				if (fromId != null)
					fromDisplay += " " + fromId;
				string fromAddress = GmailUserName;
				...
				SmtpClient smtp = new SmtpClient(_gmailServer, _gmailPort);
		...
		public static void SendFromGmail(string gmailUserName, string gmailPassword, string toAddress, string subject, string messageBody, string fromDisplayName = null, bool inThread = false) 
		{
			try 
			{
				if ((fromDisplayName == null) || (fromDisplayName.Length < 1))
					fromDisplayName = gmailUserName;
				int i = fromDisplayName.IndexOf("@");
